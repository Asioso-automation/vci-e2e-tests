...
public class ZajednickaPotrosnja extends PocetnaStranica {
	...
	public void verifikujZajednickaPotrosnja() throws InterruptedException, FileNotFoundException, IOException {
		...
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(
				"//div[contains(@class, 'v-toolbar__title subtitle-2 ml-0 pl-0 text-uppercase') and starts-with(., ' Zajednička potrošnja')]")));
		...
		assertTrue(sekcijaBtnWE.getText().trim().equals("KUPCI"), "ZajednickaPotrosnja: Naziv sekcije nije dobar!");
		assertTrue(stranicaBtnWE.getText().trim().equals("ZAJEDNIČKA POTROŠNJA"), "ZajednickaPotrosnja: Naziv stranice nije dobar!");
		assertTrue(naslovStraniceWE.getText().trim().equals("ZAJEDNIČKA POTROŠNJA"), "ZajednickaPotrosnja: Naziv stranice nije dobar!");
		assertTrue(brojKolona().size() == 8, "ZajednickaPotrosnja: Broj kolona nije dobar! ");
	}
