...
public class TefObrazac extends PocetnaStranica {
	...
	public void verifikujTefObrazac() throws InterruptedException, FileNotFoundException, IOException {
		...
		assertTrue(sekcijaBtnWE.getText().trim().equals("OBRAČUN"), "TefObrazac: Naziv sekcije nije dobar!");
		assertTrue(stranicaBtnWE.getText().trim().equals("TEF OBRAZAC"), "TefObrazac: Naziv stranice nije dobar!");
		assertTrue(naslovStraniceWE.getText().trim().equals("TEF OBRAZAC"), "TefObrazac: Naziv stranice nije dobar!");
		assertTrue(brojKolona().size() == 5, "TefObrazac: Broj kolona nije dobar!");
	}
